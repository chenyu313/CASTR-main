...
class RSG(object):
    ...
    # 获取入度邻域
    def get_in_neighbors(self,biG, head, tail):
        # 头实体入度邻域
        neighbors1 = biG.predecessors(head)
        # 尾实体入度邻域
        neighbors2 = biG.predecessors(tail)
        return set(list(neighbors1)+list(neighbors2))
    
    # 获取出度邻域
    def get_out_neighbors(self,biG, head, tail):
        # 头实体出度邻域
        neighbors1 = biG.successors(head)
        # 尾实体出度邻域
        neighbors2 = biG.successors(tail)
        return set(list(neighbors1)+list(neighbors2))

    def get_structure_similarity(self, biG, triple1, triple2):
        # 获得入度结构相似度
        neighbors1_in = self.get_in_neighbors(biG,triple1[0],triple1[1])
        neighbors2_in = self.get_in_neighbors(biG,triple2[0],triple2[1])
        ...
        # 获得出度结构相似度
        neighbors1_out = self.get_out_neighbors(biG,triple1[0],triple1[1])
        neighbors2_out = self.get_out_neighbors(biG,triple2[0],triple2[1])
...
if __name__ == '__main__':
    ...
    data_dir = os.path.join(os.getcwd(), 'data_preprocessed', args.task, 'RSG.txt') # 处理后RSG保存地址
