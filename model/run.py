...
import sys
sys.path.append('.')
from utils.load_data import MyDataset, collate_fn
from utils.vocab_reader import Vocabulary
from model.carst_model import CARST
from utils.preprocess_ind_data import filter_repeat_path
from sklearn.metrics import average_precision_score, roc_auc_score
from utils.negative_sampling_text_sim import convert_neg_sample
...
log_file = 'logs/Text/FB15k-237-V2.log'
...
parser.add_argument('--ablation', type=int, default=3, help='0: full model; 1: mask structure; 2: mask text; 3: mask Cross-attention')

args = parser.parse_args()


def predict_grail_rank(model, device, neg_path, is_eval=False):
    ...
    logger.info('hits10: %f , hits5: %f , hits1: %f , mrr: %f' % (hits10, hits5, hits1, mrr))
    return hits10 
...
def main(args):
    ...
    if args.do_train:
        ...
        model = CARST(config=config, device=device)
        ...
        for epoch in range(1, args.epoch+1):
            ...
            with torch.no_grad():     
                ...
                if True:
                    ...
                    '''
                    #torch.save(model, save_path)  # 暂时注释，不保存
                    ...
                    eval_performance = predict_grail_rank(
                            model=model, device=device, neg_path=args.neg_save_path_test, is_eval=True)
...
if __name__ == '__main__':
    ...
    args.do_test = True
    args.raw_train = os.path.join(os.getcwd(), 'dataset', args.task, 'train.txt')
    args.raw_test = os.path.join(os.getcwd(), 'dataset', args.task, 'test.txt')
    args.raw_valid = os.path.join(os.getcwd(), 'dataset', args.task, 'valid.txt')
    ...
    args.RSG_dir = os.path.join('data_preprocessed', args.task, 'RSG2.txt')
